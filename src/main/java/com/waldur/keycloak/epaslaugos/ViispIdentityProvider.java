...
public class ViispIdentityProvider extends AbstractIdentityProvider<IdentityProviderModel> {
	...
	public Response performLogin(AuthenticationRequest request) {
		...
		try {
			...
			ClientSessionCode<AuthenticationSessionModel> clientSessionCode = new ClientSessionCode<>(session,
					request.getRealm(), authSession);
			String relayState = clientSessionCode.getOrGenerateCode();
			String encodedState = IdentityBrokerState.decoded(relayState, authSession.getClient().getId(),
					authSession.getClient().getClientId(), authSession.getTabId(), null).getEncoded();
			...
			LOG.info("Auth session client (client) ID: {}",
					authSession.getClient() != null ? authSession.getClient().getClientId() : "null");
			LOG.info("Auth session client ID: {}",
			...
			LOG.info("SSO Session Max Lifespan Remember Me: {} seconds", realm.getSsoSessionMaxLifespanRememberMe());
			LOG.info("SSO Session Idle Timeout Remember Me: {} seconds", realm.getSsoSessionIdleTimeoutRememberMe());
			LOG.info("Client Session Idle Timeout: {} seconds", realm.getClientSessionIdleTimeout());
			LOG.info("Client Session Max Lifespan: {} seconds", realm.getClientSessionMaxLifespan());
			LOG.info("Access Token Lifespan: {} seconds", realm.getAccessTokenLifespan());
			LOG.info("Access Token Lifespan for Implicit Flow: {} seconds",
					realm.getAccessTokenLifespanForImplicitFlow());
			LOG.info("Access Code Lifespan: {} seconds", realm.getAccessCodeLifespan());
			LOG.info("Access Code Lifespan User Action: {} seconds", realm.getAccessCodeLifespanUserAction());
			LOG.info("Access Code Lifespan Login: {} seconds", realm.getAccessCodeLifespanLogin());
			LOG.info("Action Token Generated By Admin Lifespan: {} seconds",
					realm.getActionTokenGeneratedByAdminLifespan());
			LOG.info("Action Token Generated By User Lifespan: {} seconds",
			...
			String ticketId = xmlClient.requestAuthenticationTicket(callbackUrl, viispConfig.getServiceId(),
					viispConfig.isTestMode(), encodedState);
	...
	private String createTicketSubmitPage(String ticketId) {
		return String.format("<!DOCTYPE html>" + "<html><head>" + "<meta charset='utf-8'>"
				...
				+ "<input type='hidden' name='ticket' value='%s'/>" + "<noscript>"
	...
	protected static class ViispEndpoint {
		...
		@POST
		@Path("/")
		public Response handleCallback(@FormParam("ticket") String ticket, @FormParam("customData") String customData,
			...
			try {
				...
				LOG.info("About to call callback.getAndVerifyAuthenticationSession() at timestamp: {}",
				...
				try {
					...
					LOG.info("callback.getAndVerifyAuthenticationSession() completed at timestamp: {}",
							System.currentTimeMillis());
				} catch (Exception e) {
					LOG.error("callback.getAndVerifyAuthenticationSession() threw exception: {}", e.getMessage(), e);
					throw e;
				}
				...
				if (authSession == null) {
					...
					LOG.error("Possible reasons: session timeout, wrong state/tab ID, or session was already consumed");
					LOG.error("Realm SSO session max lifespan: {} seconds", realm.getSsoSessionMaxLifespan());
					LOG.error("Realm SSO session idle timeout: {} seconds", realm.getSsoSessionIdleTimeout());
				...
				LOG.info("Auth session ID: {}", authSession.getParentSession().getId());
				LOG.info("Auth session client: {}",
						authSession.getClient() != null ? authSession.getClient().getClientId() : "null");
				LOG.info("Auth session parent session timestamp: {}", authSession.getParentSession().getTimestamp());
				LOG.info("Time elapsed since creation: {} seconds",
						(System.currentTimeMillis() / 1000) - authSession.getParentSession().getTimestamp());
				LOG.info("Auth note VIISP_TICKET_ID: {}", authSession.getAuthNote("VIISP_TICKET_ID"));
				LOG.info("Auth note VIISP_STATE: {}", authSession.getAuthNote("VIISP_STATE"));
				LOG.info("Auth note VIISP_STATE_COPY: {}", authSession.getAuthNote("VIISP_STATE_COPY"));
				LOG.info("Auth note VIISP_TIMESTAMP: {}", authSession.getAuthNote("VIISP_TIMESTAMP"));
				if (authSession.getAuthNote("VIISP_TIMESTAMP") != null) {
					...
					LOG.info("Time since VIISP redirect: {} ms ({} seconds)", elapsedMs, elapsedMs / 1000);
				}
